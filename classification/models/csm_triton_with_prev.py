...
def cross_scan_fn(x: torch.Tensor, in_channel_first=True, out_channel_first=True, one_by_one=False, scans=0):
    ...
    CSF = CrossScanTritonF if WITH_TRITON and x.is_cuda else CrossScanF
...
def cross_merge_fn(y: torch.Tensor, in_channel_first=True, out_channel_first=True, one_by_one=False, scans=0):
    ...
    # x: (B, C, H * W) | (B, H * W, C) | (B, 4, C, H * W) | (B, H * W, 4, C)
    # scans: 0: cross scan; 1 unidirectional; 2: bidirectional;
    CMF = CrossMergeTritonF if WITH_TRITON and y.is_cuda else CrossMergeF
    return CMF.apply(y, in_channel_first, out_channel_first, one_by_one, scans)


#################################
# previous version, will be deleted... =================================================================
#################################
...
if False:
    class CrossScanTritonF(torch.autograd.Function):
        ...
        def forward(ctx, x: torch.Tensor, channel_first=True):
            ...
            ctx.shape = (B, C, H, W)
            ctx.triton_shape = (BC, BH, BW, NC, NH, NW)
            y = x.new_empty((B, 4, C, H, W)) if channel_first else x.new_empty((B, H, W, 4, C))
            triton_cross_scan_flex[(NH * NW, NC, B)](x.contiguous(), y, (0 if channel_first else 1), (0 if channel_first else 1), 0, 0, BC, BH, BW, C, H, W, NH, NW)
            return y.view(B, 4, C, -1) if channel_first else y.view(B, -1, 4, C)
        ...
        def backward(ctx, y: torch.Tensor):
            ...
            triton_cross_scan_flex[(NH * NW, NC, B)](x, y, (0 if channel_first else 1), (0 if channel_first else 1), 1, 0, BC, BH, BW, C, H, W, NH, NW)
            return x, None
