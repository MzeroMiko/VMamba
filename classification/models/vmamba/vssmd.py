...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ss2d = SS2D(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, expand=1, **kwargs)
        self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        
        mlp_hidden_dim = int(hidden_dim * mlp_ratio)
        self.norm2 = norm_layer(hidden_dim)
        self.mlp = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=nn.SiLU, drop=drop)
        self.drop_path2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()

    def forward(self, input: torch.Tensor):
        x = input
        x = x + self.drop_path1(self.ss2d(self.norm1(x)))
