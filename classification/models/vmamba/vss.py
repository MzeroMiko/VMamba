...
class SSBlock(nn.Module):
    def __init__(
        ...
        dim: int,
        drop_path: float = 0.0,
        norm_layer: Callable[..., nn.Module] = partial(cLayerNorm, eps=1e-6, data_format="channels_first"),
        **kwargs,
    ):
        super().__init__()
        self.norm = norm_layer(dim)
        self.attn = SS2D(dim, **kwargs)
...
class VSS(nn.Module):
    def __init__(self, in_chans=3, patch_size=4, num_classes=1000, 
            depths=[3, 3, 9, 3], dims=96,
        ...
        stem = nn.Sequential(
            ...
            cLayerNorm(dims[0], eps=1e-6, data_format="channels_first")
        )
        self.downsample_layers.append(stem)
        for i in range(3):
            downsample_layer = nn.Sequential(
                cLayerNorm(dims[i], eps=1e-6, data_format="channels_first"),
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[SSBlock(
                    ...
                    norm_layer=partial(cLayerNorm, eps=1e-6, data_format="channels_first"),
...
if True:
    ...
    @register_model
    def vss_tiny(**kwargs):
        model = MODEL(depths=[2, 2, 9, 2], dims=96, **kwargs)
        return model

    @register_model
    def vss_small(**kwargs):
        model = MODEL(depths=[2, 2, 27, 2], dims=96, **kwargs)
        return model

    @register_model
    def vss_base(**kwargs):
        model = MODEL(depths=[2, 2, 27, 2], dims=128, **kwargs)
        return model
