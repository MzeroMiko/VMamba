...
void selective_scan_bwd_launch(SSMParamsBwd &params, cudaStream_t stream) {
    BOOL_SWITCH(params.seqlen % (kNThreads * kNItems) == 0, kIsEvenLen, [&] {
        BOOL_SWITCH(params.delta_softplus, kDeltaSoftplus, [&] {
            constexpr int _kNRows_ = 1;
            using Ktraits = Selective_Scan_bwd_kernel_traits<kNThreads, kNItems, _kNRows_, kIsEvenLen, kDeltaSoftplus, input_t, weight_t>;
            // TODO: check this
            ...
            // printf("smem_size = %d\n", kSmemSize);
            dim3 grid(params.batch, params.dim / _kNRows_);
